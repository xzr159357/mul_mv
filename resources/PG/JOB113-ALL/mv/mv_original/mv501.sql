...
from comp_cast_type, complete_cast, title
 where (comp_cast_type.id = complete_cast.subject_id) And (comp_cast_type.kind = 'completeverified') And (complete_cast.status_id = comp_cast_type.id) And (title.id = complete_cast.movie_id)
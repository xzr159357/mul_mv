...
def get_all_yarn_logs(spark_history_path='/spark2-history'):
    (ret, out, err) = run_cmd(['/home/daily/spark_tune/environment/hadoop-3.1.1/bin/hdfs', 'dfs', '-ls', spark_history_path])
    yarn_logs_timestamp = {}
...
# get one sql plan
def get_one_sql_plan(plan_file, app_id):
    ...
    if ret != 0:
        ...
        print("Generate Json log file success!")
    ...
    tmpPath = plan_file
    tmpQueryPath = tmpPath + f"{app_id}_query.sql"
    tmpNodePath = tmpPath + f"{app_id}_node.lz4"
    tmpPlanPath = tmpPath + f"{app_id}_plan.lz4"
    tmpDotPath = tmpPath + f"{app_id}_dot.lz4"
